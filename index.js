...
function editHtml() {
	for (let i = 0; i < bosses.length; i++) {
		...
		bossesContainer.children[i].children[1].children[0].children[1].textContent =
			bosses[i].difficulty + " " + bosses[i].name;

		//finds images for bosses in html
		bossesContainer.children[i].children[1].children[0].children[0].src =
			"images/" + bosses[i].name.replace(/\s+/g, "_").toLocaleLowerCase() + ".png";
		...
		//gives id to inputs on right side of html
		bossesContainer.children[i].children[1].children[2].children[0].setAttribute(
			"id",
			"characters" + bosses[i].name.replace(/\s+/g, "") + bosses[i].difficulty
		);

		//gives id to inputs on left side of html
		bossesContainer.children[i].children[0].setAttribute(
			"id",
			"charactersInParty" +
				bosses[i].name.replace(/\s+/g, "") +
				bosses[i].difficulty
		);
	}
}
...
for (let i = 0; i < bosses.length; i++) {
	bossesContainer.children[i].children[0].addEventListener("change", (event) => {
		...
		// updates mesos value of boss crystal in html
		bossesContainer.children[i].children[1].children[1].textContent =
			type + ": " + bosses[i].mesosChanged.toLocaleString() + " mesos";
...
function sortAndAdd() {
	...
	for (let i = 0; i < bosses.length; i++) {
		...
		if (counts[bosses[i].mesosChanged + bosses[i].mesosUniqueAdd] === undefined) {
			...
			imgPath.innerHTML =
				"<img src='images/grey.png' alt='' class='bossCrystal' />";
		} else {
			textPath.textContent =
				"x " + counts[bosses[i].mesosChanged + bosses[i].mesosUniqueAdd];
			imgPath.innerHTML =
				"<img src='images/blue.png' alt='' class='bossCrystal' />";
		}
	}
	storeInfo();
...
function reset() {
	...
	for (let i = 0; i < bosses.length; i++) {
		...
		bossesContainer.children[i].children[1].children[1].textContent =
			bosses[i].type +
			": " +
			Math.floor(bosses[i].mesos).toLocaleString() +
			" mesos";

		//boss crystal amount and image reset
